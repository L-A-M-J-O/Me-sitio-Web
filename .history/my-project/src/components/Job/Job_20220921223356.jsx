import React from 'react'

import '../Job/Job.css'

function Job() {
  return (
    <div id='job'>
        <p></p>
       <div className='row card-job-main align-items-center'>
        <div className='col-12 col-sm-12 col-md-4 col-lg-4'>
          <div className="card-job">
            <div className='row logo-and-name align-items-center justify-content-center mb-3'>
              <div className="col-4">
                      <div className='block'>
                        <div className='box-job'>
                          <a href=''><img src='https://media-exp1.licdn.com/dms/image/C4E0BAQHVWeqmIAjzAw/company-logo_200_200/0/1522233689880?e=2147483647&v=beta&t=sNf-YB9yxGLVqyLa_xykMX7PCG1eaWAJ9LH2kuqItHU' alt='Booststrap V5'/></a>
                        </div>
                  </div>
              </div>
              <div className='col-6'>
                <h4>Backbone Latam</h4>
              </div>
            </div>    
          <div className="card-body">
            <p className="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
          </div>
        </div>
        </div>
        <div className='col-12 col-sm-12 col-md-4 col-lg-4'>
          <div className="card-job">
            <div className='row logo-and-name align-items-center justify-content-center mb-3'>
              <div className="col-4">
                    <div className='block'>
                      <div className='box-job'>
                        <img src='https://uploads-ssl.webflow.com/62cc216ce23f79c10bc88169/62d0486c8fb98db540881355_header.png' alt='Booststrap V5'/>
                      </div>
                    </div>
              </div>
              <div className='col-5'>
                <h4>No Country</h4>
              </div>
            </div>
          <div className="card-body">
            <p className="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
          </div>
        </div>
        </div>
       </div>
    </div>
  )
}

export default Job